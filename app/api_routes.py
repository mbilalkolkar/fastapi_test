...
def create_todo_api(item: TodoCreate, db: Session = Depends(get_db)):
...
def update_todo_api(item_id: int, item: TodoCreate, db: Session = Depends(get_db)):
    db_item = get_todo(db=db, item_id=item_id)
    if db_item is None:
        raise HTTPException(status_code=404, detail="Todo not found")
...
def delete_todo_api(item_id: int, db: Session = Depends(get_db)):
    db_item = get_todo(db=db, item_id=item_id)
    if db_item is None:
        raise HTTPException(status_code=404, detail="Todo not found")
