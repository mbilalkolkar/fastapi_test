...
def create_todo(db: Session, item: Todo):
    db_item = DBTodo(message=item.message, tag=item.tag, is_completed=item.is_completed)
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item

def update_todo(db: Session, db_item: DBTodo, item: Todo):
